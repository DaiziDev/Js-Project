...
button_code_bar.onclick = function(){
    if(text.value.length > 0){
      ...
      //Gerating the code bar na
      box.innerHTML = "<svg id='barcode'></svg>";
      JsBarcode("#barcode", text.value);
      box.style.border = '1px solid #999'
      pdf_box.innerHTML = '<button onclick="genererPDF()">Download Code</button>'
      ...
      pdf_box.style.display="flex"
        ...
        pdf_box.box.style.display = "none";
      }
    }
    else{
        box.style.border = "0";
        box.innerHTML='<p class="error"> Remplissez le cham!</p>';
